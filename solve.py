...
countries = [
    ...
    {'timezones': ['Asia/Dili'], 'code': 'TL', 'continent': 'Asia', 'name': 'East Timor (Timor-Leste)', 'capital': 'Dili'},
...
def netcat(hostname, port):
    ...
    while True:
        ...
        if len(str(data)) > 3:
            print(data)
        if 'capital' in str(data):
            location = str(data)[25:-4]
            capital = 'NONE'
            for country in countries:
                if country['name'] == location:
                    capital = country['capital']
            for state in states:
                for state, cap in iter(state.items()):
                    if state == location:
                        capital = cap
            s.send(bytes(capital, 'utf8'))

    print("Connection closed.")
    s.close()


netcat('chall.2019.redpwn.net', 6001)